...
void Outputfile(string, int, double, vec, vec, vec, vec, vec);
void Verlet( int, double, double);
void Euler( int, double, double);

// This function write results into file.
void Outputfile(string filename, int num_steps, double t_end, vec t, vec x, vec y, vec vx, vec vy){
    string fileout = filename;
    string size = to_string(t_end);
    fileout.append("_"+size+".txt");
    ofstream file(fileout);
    file << "time(yr) " << setw(5) << "x" << setw(10) << "y" << setw(10) << "vx"<< setw(10) << "vy"  << endl;
    cout << setiosflags(ios::fixed);
    for (int i = 0; i < num_steps; i++){
        file << fixed << setprecision(5) << t(i) << setw(10) << x(i) << setw(10) << y(i) << setw(10) << vx(i) << setw(10) << vy(i)  << endl;
    }
    file.close();
}
...
void Verlet(int num_steps, double t_end, double pi){
    ...
    Outputfile("Verlet", num_steps, t_end, t, x, y, vx, vy);
}
...
void Euler(int num_steps, double t_end, double pi){
    ...
    for (int i = 1; i < num_steps; i++){
        x(i) = x(i-1) + h * vx(i-1);
        y(i) = y(i-1) + h * vy(i-1);
        vx(i) = vx(i-1) - fac * x(i)/(r*r*r);
        vy(i) = vy(i-1) - fac * y(i)/(r*r*r);
        r = sqrt(x(i) * x(i) + y(i) * y(i));
        t(i) = t(i-1) + h;
    }
    //Write into file "Euler_XX"
    Outputfile("Euler", num_steps, t_end, t, x, y, vx, vy);
}
